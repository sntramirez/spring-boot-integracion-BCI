...
public class UsuarioService {
	...
	public UsuarioResponse registarUsuario(UsuarioRequest nuevoUsuario) {
		...
		return UsuarioMapper.INSTANCE.mapToDto(usuario);
	}
	...
	public UsuarioResponse actulizarUsuario(UsuarioRequest nuevoUsuario) {
		...
		usuarioUpdate.setPhones(usuario.getPhones());

		/*
		 * List<Telefono> dato = telefonoService.findByIdUsu(usuarioUpdate.getId());
		 * dato.forEach(dat->{ telefonoService.eliminar(dat); });
		 */

		usuarioUpdate = usuarioRepository.save(usuarioUpdate);

		return UsuarioMapper.INSTANCE.mapToDto(usuarioUpdate);
	}
